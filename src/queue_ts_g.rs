...
impl<T> Node<T> {
    pub fn new_with_elem(item: T) -> Self {
        Self {
            item: Some(item),
            next: AtomicPtr::new(ptr::null_mut()),
        }
    }
    pub fn new_without_elem() -> Self {
        Self {
            item: None,
            next: AtomicPtr::new(ptr::null_mut()),
        }
...
impl<T> Default for LinkedQueue<T> {
    fn default() -> Self {
        let header = Box::new(Node::new_without_elem());
...
impl<T> LinkedQueue<T> {
    ...
    pub fn push(&self, item: T) {
        let new_node = Box::new(Node::new_with_elem(item));
